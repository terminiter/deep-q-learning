...
class Phi(object):
    def __init__(self, skip_every, dtype=np.float32):
        self.prev_cropped = np.zeros((80, 80), dtype=dtype)
        self.prev_frames = [np.zeros((80, 80), dtype=dtype), np.zeros((80, 80), dtype=dtype), np.zeros((80, 80), dtype=dtype), np.zeros((80, 80), dtype=dtype)]
        self.frame_count = -1
    ...
    def __call__(self, state):
        ...
        cropped = measure.block_reduce((np.reshape(state.astype(self.dtype), (210, 160))[35:-15, :]), (2, 2), func=np.max)
